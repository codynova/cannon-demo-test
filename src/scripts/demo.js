...
const Demo = function (options) {
	...
	let gui = null;
	let smoothie = null;
	let smoothieCanvas = null;
	const scenePicker = {};

	const three_contactpoint_geo = new THREE.SphereGeometry(0.1, 6, 6);
	const particleGeo = this.particleGeo = new THREE.SphereGeometry(1, 16, 8);

	// Material
	const materialColor = 0xdddddd;
	const solidMaterial = new THREE.MeshLambertMaterial({ color: materialColor });
	...
	const contactDotMaterial = new THREE.MeshLambertMaterial({ color: 0xff0000 });
	const particleMaterial = this.particleMaterial = new THREE.MeshLambertMaterial({ color: 0xff0000 });

	// Geometry caches
	const contactMeshCache = new GeometryCache(function () {
		return new THREE.Mesh(three_contactpoint_geo, contactDotMaterial);
	});
	const cm2contactMeshCache = new GeometryCache(function () {
	...
	const bboxMeshCache = new GeometryCache(function () {
		return new THREE.Mesh(bboxGeometry, bboxMaterial);
	});
	const distanceConstraintMeshCache = new GeometryCache(function () {
		const geometry = new THREE.Geometry();
		geometry.vertices.push(new THREE.Vector3(0, 0, 0));
		geometry.vertices.push(new THREE.Vector3(1, 1, 1));
		return new THREE.Line(geometry, new THREE.LineBasicMaterial({ color: 0xff0000 }));
	});
	const p2pConstraintMeshCache = new GeometryCache(function () {
		const geometry = new THREE.Geometry();
		geometry.vertices.push(new THREE.Vector3(0, 0, 0));
		geometry.vertices.push(new THREE.Vector3(1, 1, 1));
		return new THREE.Line(geometry, new THREE.LineBasicMaterial({ color: 0xff0000 }));
	});
	const normalMeshCache = new GeometryCache(function () {
		const geometry = new THREE.Geometry();
		geometry.vertices.push(new THREE.Vector3(0, 0, 0));
		geometry.vertices.push(new THREE.Vector3(1, 1, 1));
		return new THREE.Line(geometry, new THREE.LineBasicMaterial({ color: 0x00ff00 }));
	});
	const axesMeshCache = new GeometryCache(function () {
	...
	const renderModes = [ 'solid', 'wireframe' ];
	...
	let light; let scene; let ambient; let stats; let
		info;
	...
	function updateVisuals () {
		...
		if (settings.contacts) {
			...
			for (var ci = 0; ci < world.contacts.length; ci++) {
				for (var ij = 0; ij < 2; ij++) {
					...
					var b = ij === 0 ? c.bi : c.bj;
					var r = ij === 0 ? c.ri : c.rj;
		...
		if (settings.constraints) {
			...
			for (var ci = 0; ci < world.constraints.length; ci++) {
				...
				const nc = c.equations.normal;

				var bi = nc.bi; var bj = nc.bj; var
			...
			for (var ci = 0; ci < world.constraints.length; ci++) {
				...
				var bi = n.bi; var bj = n.bj; const relLine1 = p2pConstraintMeshCache.request(); const relLine2 = p2pConstraintMeshCache.request(); const
	...
	let camera; let controls; let
		renderer;
	let container;
	const NEAR = 5; const
		FAR = 2000;
	let sceneHUD; let cameraOrtho; let
		hudMaterial;
	...
	function init () {
		...
		world.addEventListener('postStep', function (evt) {
			for (const label in world.profile) {
		...
		if (window.dat != undefined) {
			...
			// Render mode
			const rf = gui.addFolder('Rendering');
			...
			wf.add(settings, 'quatNormalizeFast').onChange(function (fast) {
				world.quatNormalizeFast = Boolean(fast);
			});

			// Solver folder
			const sf = gui.addFolder('Solver');
		...
		if (e.keyCode) {
			switch (e.keyCode) {
				...
				case 109: // m - toggle materials
					...
					idx++;
					idx %= renderModes.length; // begin at 0 if we exceeded number of modes
	...
	function buildScene (n) {
		...
		const num = visuals.length;

		for (let i = 0; i < num; i++) {
			world.remove(bodies.pop());
			const mesh = visuals.pop();
	...
	function GeometryCache (createFunc) {
		const that = this; const geometries = []; const
			gone = [];
...
Demo.prototype.setGlobalSpookParams = function (k, d, h) {
	...
	for (var i = 0; i < world.contactmaterials.length; i++) {
		const cm = world.contactmaterials[i];
...
Demo.prototype.shape2mesh = function (body) {
	const wireframe = this.settings.renderMode === 'wireframe';
